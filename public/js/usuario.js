    ...
    formLogin.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok && data.usuario) {
          // Guardamos el token JWT para las peticiones protegidas
          if (data.token) API.guardarToken(data.token);
          // Persistimos el usuario en localStorage para que App.init() lo reconozca
          localStorage.setItem('usuario', JSON.stringify(data.usuario));
          // Ocultar modal de auth y mostrar fila virtual antes de arrancar la app
          document.getElementById('auth-modal').style.display = 'none';
          App._mostrarFilaVirtual
            ? App._mostrarFilaVirtual(() => App.arrancarAplicación(data.usuario))
    ...
    formRegister.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok && data.usuario) {
          ...
          // Persistimos el usuario en localStorage para que App.init() lo reconozca
          localStorage.setItem('usuario', JSON.stringify(data.usuario));
          // Registro exitoso: fila virtual y luego app
          document.getElementById('auth-modal').style.display = 'none';
          App._mostrarFilaVirtual
            ? App._mostrarFilaVirtual(() => App.arrancarAplicación(data.usuario))
            : App.arrancarAplicación(data.usuario);
